<?php 
	...
	$resultado = mysqli_query ($miConexion,"SELECT * FROM proveedor WHERE codProveedor 
								LIKE '%$buscar%' or Apellido LIKE '%$buscar%' or RazonSocial 
								LIKE '%$buscar%' order by Apellido ASC");
...
<?php if ($total>0 && $buscar!='') { 
	...
	// codProveedor Apellido Nombre RazonSocial Domicilio TelFijo TelCelular Email Ciudad CodigoPostal Provincia

//Creo la tabla y lleno con la consulta
    	$dyn_table= "<table id='TablaCliente' class='table table-striped table-hover'>";
		$dyn_table.="<th>" . "Cuit/Cuil" ."</th>";  
